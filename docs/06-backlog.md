...
| 2 | 时间展示例为五段：清晨 / 白天 / 傍晚 / 夜晚 / 深夜 | `04-art.md`：早朝 / 白天 / 夕方 / 夜晚，四段 | **已定：图例不算规格，四段为准（#74）** |
| 3 | 鱼缸是亮粉与薄荷绿的**珊瑚礁**造型 | 造景方向要求圆润景石成组、不做珊瑚枝 | 待定——这一条是画面本身，不是图例 |
| 4 | 图例写「植物状态（季节变化）」 | `04-art.md` Seasonal 层：室内盆栽第一版固定 | **已定：图例不算规格，盆栽固定（#74）** |
| 5 | 图例仍列「记一笔」为第一版入口 | 已由 #65 推迟到下期 | **已定：图例不算规格，按 #65 推迟（#74）** |
